...
static CommandLineParser<Arguments> buildCmdLineParser() {
    ...
    std::vector<const CommandLineOption<Arguments> *> options{
            ...
            new ArgumentOption<Arguments>(
                    ...
                    "mode", 'm', "mode in which to run ('cuda' for CUDA, 'opencl' for OpenCL, or 'cpu' for CPU)",
                    "cuda", "MODE"),
            ...
            new ArgumentOption<Arguments>(
                    [](Arguments &state, const std::string &name) {
                        state.benchmarkDeviceName = name;
    ...
    return CommandLineParser<Arguments>(
            "XENBlocks gpu miner: CUDA is supported.",
            positional, options);
}
...
int main(int, const char *const *argv) {
    ...
    if (args.listDevices) {
        BenchmarkDirector director(argv[0], argon2::ARGON2_ID, argon2::ARGON2_VERSION_13,
                                   "24691e54afafe2416a8252097c9ca67557271475",
                                   1, 120, 1, 1,
    ...
    std::string saltDevFee = "B74Daea1d0eA8DD862828073Ffd4552DF0aAD7bA";
